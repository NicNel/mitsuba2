...
public:
    ...
    DiffuseTransmitter(const Properties &props) : Base(props) {
        m_reflectance = props.texture<Texture>("reflectance", 1.0f);
        m_transmittance = props.texture<Texture>("transmittance", .5f);
        m_flags = +BSDFFlags::DiffuseTransmission;
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        mask_t<Float> c_i = cos_theta_i > 0;
        if (any_or<true>(c_i))
            bs.wo[2] *= -1;
        ...
        bs.pdf               = abs(cos_theta_o) * math::InvPi<Float>; 
        bs.eta               = 1.f;
        bs.sampled_type      = +BSDFFlags::DiffuseTransmission;
        bs.sampled_component = 0;

        UnpolarizedSpectrum T = m_transmittance->eval(si, active);
        return { bs, select(active,
                            unpolarized<Spectrum>(T), 0.f) };
    }
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        UnpolarizedSpectrum value = m_transmittance->eval(si, active) *
                                    math::InvPi<Float> * abs(cos_theta_o);
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        Float pdf = abs(cos_theta_o) * math::InvPi<Float>;

        return select(cos_theta_i * cos_theta_o < 0.f, pdf, 0.f);
    }

    Float MaxComponentValue(UnpolarizedSpectrum values) const {
        int NSpectrumSamples = 4;
        Float m              = values[0];
...
MTS_EXPORT_PLUGIN(DiffuseTransmitter, "Diffuse Transmitter material")
